...
//!
//! All blocks are void-masked via QSMM: after AEAD encryption, the block is
...
pub fn write_slot(
    ...
    for slot in 0..MAX_SLOTS {
        ...
        match decrypt_block_masked(&key, &existing, master_secret, offset) {
            Ok(plaintext_buf) => {
...
/// Write an encrypted + void-masked block using copy-on-write semantics.
///
...
/// explicitly erases it. This enables atomic multi-block overwrites.
///
/// Returns `Ok(Some(old_offset))` if the block already existed, `Ok(None)` for fresh.
pub fn write_slot_cow(
    ...
    match (own_slot, first_available) {
        (Some(old_offset), Some(new_offset)) => {
            let encrypted = encrypt_block_masked(&key, plaintext, master_secret, new_offset)?;
            image.write_block(new_offset, &encrypted)?;
            image.claim_offset(new_offset);
            Ok(Some(old_offset))
        }
        (Some(old_offset), None) => {
            let encrypted = encrypt_block_masked(&key, plaintext, master_secret, old_offset)?;
...
/// Read and decrypt a void-masked block, trying all slot candidates.
///
...
pub fn read_slot(
    ...
    for slot in 0..MAX_SLOTS {
        ...
        match decrypt_block_masked(&key, &raw, master_secret, offset) {
            Ok(plaintext) => {
